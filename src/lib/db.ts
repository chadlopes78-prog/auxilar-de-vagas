...
const forceNeon = Boolean(process.env.NETLIFY);
 ...
 */
export const dbSource: DbSource = databaseUrl || forceNeon ? "neon" : "pglite";
...
function createNeonSql(): Promise<Sql> {
  globalRef.__pgSqlPromise__ ??= (async () => {
    ...
    if (!url) {
      throw new Error("DATABASE_URL em falta. Ligue o Supabase no Netlify e volte a publicar.");
    }
    const { pgPoolOptions } = await import("./pg-options");
