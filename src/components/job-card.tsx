...
export function JobCardView({
  ...
  return (
    <article
      className="group grid cursor-pointer grid-cols-[48px_1fr_auto] gap-3 rounded-xl border border-border bg-surface p-4 transition-shadow duration-150 hover:shadow-[0_8px_24px_rgba(20,34,28,0.08)] md:grid-cols-[56px_1fr_auto] md:gap-4"
      onClick={() => navigate({ to: "/jobs/$jobId", params: { jobId: String(job.id) } })}
    >
      <div className="grid size-12 place-items-center rounded-xl bg-primary-soft text-sm font-semibold text-primary md:size-14">
        {initials(job.companyName)}
      </div>
      <div className="min-w-0">
        <h3 className="font-sans text-base font-semibold uppercase tracking-tight">{job.title}</h3>
        <p className="mt-0.5 text-sm text-muted">{job.companyName}</p>
        <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted">
          <span className="inline-flex items-center gap-1">
            <MapPin className="size-3.5" />
            {loc || "Localização a confirmar"}
          </span>
          <span>
      ...
      <div className="flex flex-col items-stretch gap-2" onClick={(e) => e.stopPropagation()}>
        <button
          type="button"
          className="inline-flex min-h-11 items-center justify-center gap-1.5 rounded-[10px] border border-border px-3 text-xs font-medium hover:bg-bg"
